...
const base: Offer = {
  ...
  sourceUrl: 'https://example.invalid/a', unitPriceCents: 650, priceUnit: 'g', shippingCents: 499,
...
describe('pricing', () => {
  ...
  it('ranks current shipping offers by total price for requested grams', () => {
    ...
    const ranked = rankOffers([base, freeShipping], { mode: 'shipping', quantity: 10 }, new Date('2026-07-22T18:00:00.000Z'))
    expect(ranked.map(({ offer, totalPriceCents }) => [offer.id, totalPriceCents])).toEqual([
      ['offer-b', 6750], ['offer-a', 6999]
    ])
  })

  it('calculates totals from a quantity whose display unit belongs to the offer', () => {
    const extract = { ...base, form: 'extract' as const, priceUnit: 'ml' as const, unitPriceCents: 710 }
    ...
    expect(ranked.totalPriceCents).toBe(1_919)
    expect(ranked.offer.priceUnit).toBe('ml')
  })
})
